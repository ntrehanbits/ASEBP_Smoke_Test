# Update Review Request with submit button functionality
import time
import pyperclip
import pytest
...
@pytest.mark.test
def test_Update_Review_Submit_button(test_login):
    ...
    try:
        ...
        Status_Field.click()
        # Selecting value from status field dropdown using Action Chains
        action = ActionChains(driver)
        action.key_down(Keys.ARROW_DOWN)
        action.key_down(Keys.ARROW_DOWN)
        action.key_down(Keys.ENTER).perform()
    ...
    # Click on the Selected Review request after applying filters
    try:
        str1 = driver.find_element_by_xpath("//tbody/tr[1]/td[1]/div//a")
        str1.click()
    except:
        print("Unable to click on the selected Review Request")
    ...
    try:
        ...
        print("Update Review Request button is not clicked")
    # Scroll down and click on Add document link
    driver.execute_script("window.scrollTo(0,document.body.scrollHeight)")

    # Add new Title for the Review Request
    try:
        ...
        Title.clear()
        Title.send_keys("Updated Automated Review Request Title")
    except:
        print("Unable to add new Title")

    # click on Add Reviewer link and add new Reviewer in the list
    try:
        Add_Reviewer_link = driver.find_element_by_xpath("//a[contains(text(),'Add Reviewer')]")
        Add_Reviewer_link.click()
        # time.sleep(2)
        Reviewer_2 = driver.find_element_by_xpath("//div[@class='ContentLayout---content_layout ContentLayout---padding_less']//tr[3]//div/div/input")
        Reviewer_2.send_keys("Raw user 02")
        time.sleep(2)
        action = ActionChains(driver)
        action.key_down(Keys.ENTER).perform()
    except:
        print("Unable to add new Reviewer in Reviewer list")
